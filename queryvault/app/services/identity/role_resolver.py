"""
role_resolver.py -- Role Inheritance & Clearance Resolution (ZT-002)
====================================================================

Implements a directed acyclic graph (DAG) for role inheritance using BFS
traversal.  In production this would query a Neo4j graph; for the MVP the
inheritance tree and clearance mappings are hardcoded from the Apollo
Hospitals RBAC specification.
...
17 primary roles covering Clinical, Nursing, Business/Revenue,
Administrative, IT, Compliance, and Research hierarchies.
"""
...
logger = logging.getLogger("queryvault.identity.role_resolver")


# ─────────────────────────────────────────────────────────
# ROLE INHERITANCE GRAPH (mock Neo4j DAG)
# ─────────────────────────────────────────────────────────
...
# MFA absent -> reduce clearance by this many levels
...
# ─────────────────────────────────────────────────────────
# Downstream policy resolution uses lowercase pipeline role names
# (doctor, nurse, billing_staff, etc.).  This map translates the
# Azure AD role into the pipeline role for policy matching.
...
class RoleResolver:
    """
    Expands direct roles into effective roles via DAG inheritance,
    computes clearance level, and applies MFA-based sensitivity cap.
    ...
    def resolve(self, direct_roles: list[str], mfa_verified: bool) -> ResolvedRoles:
        ...
        Returns:
            ResolvedRoles with effective_roles, clearance, sensitivity_cap,
            domain, and bound_policies.
        """
        ...
        # -- 2b. Add pipeline roles (lowercase names used by policy resolution) --
